...
export default function Home() {
  ...
  const { upload, isPending, error } = useUploadFile({
    ...
    onUploadComplete: async (data) => {
      ...
      // Small delay to ensure upload state clears
      setTimeout(async () => {
        // Set generating state before calling the function
        ...
        await generateAltText(
          "https://assets.moinulmoin.com/" + data.file.objectKey,
        );
      }, 100);
    },
    onError: (error) => {
  ...
  useNavigationWarning({
    ...
    message:
      "Are you sure you want to leave? Your upload progress will be lost.",
  });
  ...
  const handleImageChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      if (credits && credits.remaining === 0) {
        toast.error("No credits remaining! Try again tomorrow.");
        return;
      }
      // Create preview
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.creditsRemaining !== undefined) {
          setCredits((prev) =>
            prev ? { ...prev, remaining: data.creditsRemaining } : null,
          );
        }
      } else {
        if (response.status === 429) {
          toast.error("Daily credit limit reached! Try again tomorrow.");
        }
